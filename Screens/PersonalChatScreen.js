...
export default function GroupChatScreen() {
  ...
  useEffect(() => {
    const fetchGroupMessage = async () => {
      try {
        const freindId = await AsyncStorage.getItem('freindId');
        const userId = await AsyncStorage.getItem('userId');       
        const pageNum = 1;
        const pagesize = 20;
        const  groupMessages = 
               await connection.invoke("GetFriendMessageHistry", freindId, pageNum, pagesize);
        ...
        groupMessages.forEach(message => {
          if(!message.seen && userId !== message.senderId){
            message.seen = true;
            const id = message.id;
            const m = async(id) =>{await setMessageAsSeen(id)};
            if(!m){
  ...
  useEffect(() => {
    ...
    connection.on('RecieveMessage', (message) =>{
        setMessages((prevMessages) => [message, ...prevMessages]);
        console.log("froms reciveMessage: ", message);
    });
  }, []);
  useEffect(() => {
    updateMessageHandler((message) => {
      setMessages((prevMessages) => {
        const updatedMessages = prevMessages.map((m) => {
          if (m.id === message.id) {
            if(JSON.stringify(m) === JSON.stringify(message)){
              return m;
            }
            return message;
          }
  ...
  const messageHandler = async (messageId) => {
    ...
    console.log("from messageHandler: ", messageId); 
  }
  return (
    < >
